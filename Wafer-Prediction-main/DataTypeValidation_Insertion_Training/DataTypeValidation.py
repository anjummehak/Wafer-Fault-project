...
class DatabaseOperations:
    def __init__(self):
        self.base_path = 'Training_Database/'
        self.invalid_data_path = "Training_Raw_files_validated/Bad_Raw"
        self.valid_data_path = "Training_Raw_files_validated/Good_Raw"
        self.logger = App_Logger()

    def connect_to_database(self, db_name):
        try:
            conn = sqlite3.connect(self.base_path + db_name + '.db')
            with open("Training_Logs/DataBaseConnectionLog.txt", 'a+') as log_file:
    ...
    def create_table(self, db_name, column_definitions):
        try:
            ...
            if cursor.fetchone()[0] == 1:
                conn.close()
                with open("Training_Logs/DbTableCreateLog.txt", 'a+') as log_file:
                    self.logger.log(log_file, "Table already exists")
                return
    ...
    def insert_good_data(self, db_name):
        try:
            ...
            with open("Training_Logs/DbInsertLog.txt", 'a+') as log_file:
    ...
    def export_data_to_csv(self, db_name):
        ...
        try:
            ...
            with open("Training_Logs/ExportToCsv.txt", 'a+') as log_file:
